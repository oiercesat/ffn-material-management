...
export function useMaterials() {
  ...
  const updateMaterial = (id: string, updates: Partial<Material>) => {
    setMaterials((prev) => prev.map((material) => (material.id === id ? { ...material, ...updates } : material)))
  }
  ...
  const getStats = () => {
    ...
    const available = materials.filter((m) => m.status === "disponible").reduce((acc, m) => acc + m.quantity, 0)
    const loaned = materials.filter((m) => m.status === "prêté").reduce((acc, m) => acc + m.quantity, 0)
